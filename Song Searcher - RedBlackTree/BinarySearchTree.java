...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected static class Node<T> {
        ...
        public BinarySearchTree.Node<T> left;

        public BinarySearchTree.Node<T> right;
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        // Update the grandparent's child pointer or the root of the tree
        if (grandparent != null) {
    ...
    public static boolean test1() {
        ...
        Node<Integer> parent = new Node<>(114514);
        Node<Integer> child = new Node<>(1);

        boolean nullParentPassed = false;
        boolean nullChildPassed = false;

        // Check for null parent
        try {
            tree.rotate(child, null);
        } catch (IllegalArgumentException e) {
            // Expected behavior
            nullParentPassed = true;
        }

        // Check for null child
        try {
            tree.rotate(null, parent);
        } catch (IllegalArgumentException e) {
            // Expected behavior
            nullChildPassed = true;
        }

        return nullParentPassed && nullChildPassed;
    ...
    public static boolean test2() {
        BinarySearchTree<Integer> tree = new BinarySearchTree<>();
        Node<Integer> parent = new Node<>(1);
        Node<Integer> child = new Node<>(114514);
